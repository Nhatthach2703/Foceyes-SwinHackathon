...
def xoa_cua_so():
    ...
    if detection.video_name!="":
        ...
        if detection.video_name[len(detection.video_name) - 5] != str(detection.TOTAL_WRONG) and review==0:
...
def reviewStart(link):
    ...
    tempTotalWrong = detection.TOTAL_WRONG
    review = 1
...
def exitReviewMode():
    ...
    review = 0
    detection.exitReviewMode(tempTotalWrong)
    
def poseMode():
    xoa_cua_so()
    window.withdraw()
    subprocess.run(['python',"interfaceM2.py"])
...
distance_frame.place(x=700, y=150)
...
timer_label = Label(distance_frame, fg='black', font=('Myriad Pro Bold Condensed', 10))
timer_label.grid(row=5, column=0, sticky='w')
safe_label = Label(distance_frame, fg='black', font=('Myriad Pro Bold Condensed', 10))
safe_label.grid(row=1, column=0, sticky='w')
ln_label = Label(distance_frame, fg='black', font=('Myriad Pro Bold Condensed', 10))
ln_label.grid(row=2, column=0, sticky='w')
rn_label = Label(distance_frame, fg='black', font=('Myriad Pro Bold Condensed', 10))
rn_label.grid(row=3, column=0, sticky='w')
incli_label = Label(distance_frame, fg='black', font=('Myriad Pro Bold Condensed', 10))
incli_label.grid(row=4, column=0, sticky='w')

poseButton = Button(distance_frame, text ="POSE MODE", command = poseMode, font=('Myriad Pro Bold Condensed', 10))
poseButton.grid(row=6, column=0, pady=20, sticky='w')
...
while running == 1:
    ...
    ln_label.config(text=detection.ln_text)
    rn_label.config(text=detection.rn_text)

    safe_text="Appropriate distance"
    safe_label.config(fg='green')
    if (detection.distance_to_camera) < 45:
        safe_text="Please stay further from the screen"
        safe_label.config(fg='red')
    
    if (detection.distance_to_camera) > 80:
        safe_text="Please stay closer to the screen"
        safe_label.config(fg='red')

    if (detection.incli_text == "Center"):
        incli_label.config(fg='green')
    else:
        incli_label.config(fg='red')

    incli_label.config(text=detection.incli_text)
