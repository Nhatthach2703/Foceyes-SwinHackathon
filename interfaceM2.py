...
import time
from datetime import datetime
import detectionM2 as detection
import calendar
import cv2 as cv
import subprocess
...
def xoa_cua_so():
    ...
    detection.cv2.destroyAllWindows()
...
#Mode 2 Posing
posing_frame = Frame(canvas)
posing_frame.place(x=700, y=150)
posing_label = Label(posing_frame, fg='black', font=('Myriad Pro Bold Condensed', 13))
posing_label.grid(row=0, column=0, sticky='w')
hipAngle_label = Label(posing_frame, fg='black', font=('Myriad Pro Bold Condensed', 10))
hipAngle_label.grid(row=1, column=0, sticky='w')
shoulderAngle_label = Label(posing_frame, fg='black', font=('Myriad Pro Bold Condensed', 10))
shoulderAngle_label.grid(row=2, column=0, sticky='w')
timer_label = Label(posing_frame, fg='black', font=('Myriad Pro Bold Condensed', 10))
timer_label.grid(row=3, column=0, sticky='w')

poseButton = Button(posing_frame, text ="FACE MODE", command = faceMode, font=('Myriad Pro Bold Condensed', 10))
poseButton.grid(row=4, column=0, pady=20, sticky='w')
...
while running == 1:
    ...
    posing_title = f'Posing'.upper()
    posing_label.config(text=posing_title)
    hipAngle_label.config(text=detection.torso_inclination_text)
    shoulderAngle_label.config(text=detection.neck_inclination_text)
